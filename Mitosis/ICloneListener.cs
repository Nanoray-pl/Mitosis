...
public interface ICloneListener : IReferenceCloneListener
{
	...
	void OnClone<T>(ICloneEngine engine, T source, ref T destination);

	void IReferenceCloneListener.OnClone<T>(ICloneEngine engine, T source, T destination) where T : class
		=> this.OnClone(engine, source, ref destination);
}
